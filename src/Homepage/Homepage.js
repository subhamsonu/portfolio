import React, { useEffect, useState } from "react";
import Navbar from "../Navbar";

function Homepage() {
  const [step, setStep] = useState(0);

  useEffect(() => {
    const timeouts = [];

    timeouts.push(setTimeout(() => setStep(1), 1000));
    timeouts.push(setTimeout(() => setStep(2), 2000));
    timeouts.push(setTimeout(() => setStep(3), 3000));
    timeouts.push(setTimeout(() => setStep(4), 4000));

    return () => timeouts.forEach((timeouts) => clearTimeout(timeouts));
  }, []);

  return (
    <>
      <div className="bg-gray-900 text-white h-screen">
        <Navbar />

        <div className="bg-gray-800 text-lightGray min-h-screen flex flex-col">
          <div className="container mx-auto px-8 py-24 ">
            {step >= 1 && (
              <p className="font-poppins text-teal text-sm ">Hi, My Name is</p>
            )}

            {step >= 2 && (
              <h1 className="text-5xl md:text-7xl font-bold text-quickSilver ">
                Subham Suar.
              </h1>
            )}

            {step >= 3 && (
              <h2 className="text-4xl md:text-6xl font-bold text-lightGray ">
                I am a Full-Stack Developer
              </h2>
            )}

            {step >= 4 && (
              <h3 className="text-3xl md:text-3xl font-semibold ">
               I bring my ideas to life through web creations for fun.
              </h3>
            )}
          </div>
        </div>
      </div>
    </>
  );
}

export default Homepage;
